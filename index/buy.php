...
if (isset($_COOKIE['age']) and $_COOKIE['age']<>0) {
	...
	switch ($_COOKIE['age']) {
		case 1:
			$compareyear=$year-1;
			$where .= " and ( p_year >= ".$compareyear."and p_month >= ".$month." ) ";
			break;
		case 2:
			$compareyear=$year-3;
			$cha=$year-1;
			$where .= " and ( p_year >= ".$compareyear." and p_month >= ".$p_month." ) and ( p_year <= ".$cha." and p_month <= ".$p_month." )";
			break;
		case 3:
			$compareyear=$year-5;
			$cha=$year-3;
...
if (isset($_GET['pagenum'])) {
	...
	setMyCookie("pagenum", 28, time() + COOKIETIME);
} 
...
foreach($list as $key => $value) {
	if(!empty($value['p_mainpic'])){
		...
		$list[$key]['p_mainpic'] = $pic[0].".".$pic[1];
	}
