...
$arr_s = array('1' => '未售罄', '2' => '已售罄');
$tpl -> assign('arr_issll', $arr_s);
...
if (isset($_GET['c'])) {
	...
	elseif ($arr_c['0'] == "s") {
		if (isset($arr_c[1])) {
			setMyCookie("issll", intval($arr_c[1]), time() + COOKIETIME);
		} 
		if (isset($_COOKIE['issll']) and $_COOKIE['issll'] == 0) {
			setMyCookie("issll", '', time() - COOKIETIME);
		} 
	} 
} 
...
$tpl -> display('default/' . $settings['templates'] . '/car_sell.html');
?>