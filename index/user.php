...
$tpl -> assign('userrentcarcounts', $userrentcarcounts);
...
elseif ($ac == 'addcar' || $ac == 'editcar') {
	...
	if (submitcheck('a')) {
		...
		$user=$db->row_select_one('member','id='.$_SESSION['USER_ID']);


		$post['p_tel']=$user['mobilephone'];
		$post['p_username']=$user['nicname'];
...
elseif ($ac == 'addrentcar' || $ac == 'editrentcar') {
	...
	if ($ac == 'addrentcar' and $userinfo['isdealer'] == 1 and $settings['rentlimit']==1 and !empty($settings['rentcount'])) {
		...
		if ($usercarcounts >= $settings['rentcount']) {
			showmsg('超出限制发布条数', "index.php?m=user&a=rentcarlist");
		} 
	}
	if (submitcheck('a')) {
		foreach (array('p_details') as $v) {
		...
		showmsg($ac_arr[$ac] . ($rs ? '成功' : '失败'), "index.php?m=user&a=rentcarlist");
	} 
	// 转向添加或修改页面
	else {
		$configure_list = array();
