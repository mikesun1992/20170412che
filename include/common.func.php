...
// 判断检测员登陆
function is_jcs_login()
{
	if (empty($_SESSION['jcs_id']) || empty($_SESSION['name'])) return false;
	return true;
}
...
// 租车车型数组
function arr_rentmodel()
{
	global $db;
	$list = $db -> row_select('rentmodel', "1=1", 's_id,s_name', 0, 'orderid asc');
	return get_array($list, 's_id', 's_name');
} 
...
function get_carlist($cid=0,$where,$num,$orderby)
{
	...
	foreach($list as $key => $value) {
		...
		if(!empty($value['p_mainpic'])){
			...
			$list[$key]['p_mainpic'] = WEB_DOMAIN.$pic[0]  . "." . $pic[1];
		}
		...
		$list[$key]['p_url'] = WEB_PATH.HTML_DIR."/buy/" . date('Y/m/d', $value['p_addtime']) . "/" . $value['p_id'] . ".html";
	} 
	return $list;
}
...
function get_comnews($sid, $num)
{
	...
	foreach($list as $key => $value) {
		...
		$list[$key]['shordetail'] = _substr(strip_tags(htmlspecialchars_decode($value['n_info'])),0,80);
 ...
 * 推荐新闻
 */
function get_picnews($sid, $num)
{
	global $db;
	$list = $db -> row_select('news', "catid=" . $sid." and isrecom=1", '*', $num, 'n_addtime desc');
	foreach($list as $key => $value) {
		$list[$key]['shorttitle'] = _substr($value['n_title'], 0, 60);
		$list[$key]['shorttitle2'] = _substr($value['n_title'], 0, 46);
		$list[$key]['shordetail'] = _substr(strip_tags(htmlspecialchars_decode($value['n_info'])),0,180);
		$list[$key]['n_addtime'] = date('Y-m-d', $value['n_addtime']);
		$list[$key]['n_url'] = WEB_PATH."/news/". date('Ym', $value['n_addtime']) . "/".$value['n_id'].".html";
	} 
	return $list;
...
//车源评估
function rapidasscars($subsubbrand,$mk,$reyear,$remonth){
	global $db;
	$arrt='';
	$smk=30;
	$sterm=180;
	...
	// 判断是否传过来综合参数
	$xiterm = round($term/12);
	$mk = $mk>($xiterm*60000)?($xiterm*60000):$mk;
	if($mk>=$smk){$gl=0.15;}else{$gl=$mk/$smk;} //超过了规定公里数 该车的残留价格为0.15 以不同的车残留价格市场而定
	if($term>=$sterm){$term=$sterm;$cx=0.15;}else{$cx=$term/$sterm;} //超过了使用年限 该车的市场价格0.15 以不同的车残留价格市场而定
    if($arrt){
        $ratio = ($arrt['0']*30/100)+($arrt['1']*25/100)+($arrt['2']*20/100)+($arrt['3']*15/100)+($arrt['4']*10/100); //综合成新率
    }else{
        $ratio = xianarrt($term,$sterm);
    }
    $newness = ((1-$gl)+(1-$cx))/2; //最后的成新率
    $price = round(ceil($newness*$ratio*$guideprice),2);
    return $price;
	
}
//如果没有$arrt值为空就加载这个数组对应的
function xianarrt($term,$sterm){
	$arr8 = array('0.8','0.7','0.6','0.5','0.4','0.3','0.25','0.2');
	$arr10 = array('0.8','0.7','0.6','0.5','0.4','0.35','0.3','0.25','0.2');
	$arr15 = array('0.8','0.7','0.65','0.6','0.55','0.5','0.45','0.4','0.35','0.3','0.27','0.24','0.22','0.2');
	$xiterm = round($term/12);
	if($xiterm>'0'){
		$xiterm = $xiterm-1;
	}
	if($sterm=='96'){
		if($xiterm>=8){return 0.2;}
		return  $arr8["$xiterm"];
	}elseif($sterm=='120'){
		if($xiterm>=10){return 0.2;}
		return $arr10["$xiterm"];
	}else{
		if($xiterm>=15){return 0.2;}
		return $arr15["$xiterm"];
	}
}
...
function display_common_cache() {
	...
	//租车车型数组
	$commoncache['rentmodellist'] = arr_rentmodel();
	//年份
	...
	//城市数组
	$commoncache['recomcitylist'] = arr_city_recom();
	//二手车行情
